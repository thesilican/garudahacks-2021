...
async function main() {
  ...
  while (true) {
    ...
    expiring = fridge
      .slice()
      .filter((x) => x.dateExpires.getTime() - now < 3 * msPerDay)
      .sort((a, b) => a.dateExpires.getTime() - b.dateExpires.getTime());
    if (fridge.find((x) => x.dateExpires.getTime() - now < 1 * msPerDay)) {
      expiringState = "red";
    } else if (
      fridge.find((x) => x.dateExpires.getTime() - now < 3 * msPerDay)
    ) {
      expiringState = "yellow";
    } else {
      expiringState = "green";
    }
    await notion.updateExpiringSoon(expiring.slice(0, 3));
    ...
    for (const recipe of recipes) {
      ...
      const expiring = recipe.ingredients.filter((x) =>
        fridge.find(
          (f) => f.name === x && f.dateExpires.getTime() - now < 3 * msPerDay
        )
      ).length;
    ...
    // Read recipes
    await notion.readRecipes().then((recipes) =>
    ...
    // Read read stores
    await notion.readStores().then((stores) =>
